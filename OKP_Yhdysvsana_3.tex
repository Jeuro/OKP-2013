...
Ohjelmointikielen tyypitys on hyvin olennaista ohjelmointikielen käytössä. Vahvalla ja heikolla tyypityksellä on omat vahvuutensa ja heikkoutensa.
\subsection{COBOL}
Vaikka COBOL näyttää ulkoapäin vahvasti tyypitetyltä ja Pythonin voisi kuvitella olevan heikosti tyypitetty --- totuus on kuitenkin päinvastainen.
\subsubsection{Alkeis- ja perustyypit}
COBOLissa muuttujat ovat tyypiltään aakkosellisia (alphabetic), numeerisia (numeric), muokattu numeerinen (numeric-edited), aakkosnumeerinen (alphanumeric), kanji ja muokattu kanji (kanji-edited) \cite{cobolintro}.
...
Michael Coughlanin COBOL Tutorialin \cite{cobolintro} mukaan kääntäjä ei tee tyyppitarkistuksia, joten COBOL on siis heikosti tyypitetty kieli. Esimerkiksi ei-numeerinen arvo on mahdollista sijoittaa numeerista tietoa sisältävään muuttujaan. Kun väärää tietotyyppiä sisältävällä muuttujalla yritetään tehdä laskutoimituksia, COBOL-ohjelma kaatuu suorituksenaikaiseen virheeseen. Muuttujat määritelläänkin siten, että niistä annetaan ikään kuin kuva, miltä niiden pitäisi näyttää --- tätä kuvaa määrittelyssä käytetty \verb+PICTURE+-lauseke, joka suomentuu kuvaksi.
...
\subsection{Python}
Pythonin tyypit eroavat huomattavasti COBOLin tyypeistä.
\subsubsection{Alkeis- ja perustyypit}
...
Ohjelmiston jaottelu eri funktioihin ja aliohjelmiin on tärkeä osa ohjelmointia. Kapseloimalla laskentaa voidaan koodissa tuoda esiin erilaisia abstraktion tasoja.
\subsection{COBOL}
COBOLissa ohjelmiston struktuuri on hyvin yksinkertainen, eikä se tarjoa samanlaisia moderneja toiminnallisuuksia kuin Python.
...
COBOLissa ei ole yleistä virheiden käsittelyyn käytettävää rakennetta, kuten \verb+try...catch+-lausetta. Virheet on siis käsiteltävä toisin --- esimerkiksi näin:
...
Esimerkissä hyväksytään käyttäjältä syöte, minkä jälkeen tarkistetaan, onko syöte numeerinen ja tulostetaan virheviesti, jos mikäli syöte ei ole numeerinen.

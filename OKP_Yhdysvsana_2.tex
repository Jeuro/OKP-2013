...
Toistoon voidaan k‰ytt‰‰ \verb+for+-toistolausetta tai ehdollista \verb+while+-lausetta. K‰ytett‰ess‰ \verb+while+-lausetta ehto tulkitaan samalla tavalla kuin ehtolauseiden yhteydess‰. Muutoin \verb+while+ toimii samalla periaatteella kuin Javassa. Monessa muussa kieless‰ esiintyv‰‰ \verb+do-while+-toistorakennetta Pythonissa ei ole.

Pythonin \verb+for+-lause muistuttaa Javan \verb+for-each+-silmukkaa, ja sill‰ voidaan iteroida iteroitavia olioita, kuten listoja, merkkijonoja ja iteraattoreita. 
...
Pythonin erikoisuutena \verb+while+- ja \verb+for+-lauseiden yhteydess‰ voidaan k‰ytt‰‰ \verb+else+-lausetta. Jos \verb+while+-silmukan lopussa on \verb+else+, \verb+else+-haaran koodi suoritetaan, kun \verb+while+:n ehto on ep‰tosi ja silmukka p‰‰ttyy \cite{pythoncompound}. Kummassakaan tapauksessa \verb+else+-haaran koodia ei suoriteta, jos silmukka p‰‰ttyy \verb+break+- tai \verb+continue+-komentoon.
